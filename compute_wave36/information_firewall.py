...
for p in HERE.glob('*.py'):
    ...
    scanned.append(str(p.relative_to(HERE.parent))); t=p.read_text().lower()
    for token in forbidden:
        if token in t: hits.append({'file':str(p.relative_to(HERE.parent)),'token':token})
signals={'information_firewall_pass':len(hits)==0}
out={'test':'information_firewall','scanned_files':sorted(scanned),'hits':hits,'signals':signals}; write('wave36_information_firewall.json',out); assert all(signals.values())
