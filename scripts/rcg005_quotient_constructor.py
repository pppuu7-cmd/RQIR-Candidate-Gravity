...
def main():
 ...
 rows += [[sp.Rational(x) for x in r] for r in cm];R=sp.Matrix(rows);qdim=R.cols-R.rank();chosen=[9,11]+[len(dreps)+i for i in c.PARENT_BASIS];aug=sp.Matrix.vstack(R,*[sp.Matrix([[1 if j==i else 0 for j in range(R.cols)]]) for i in chosen]);parent=json.loads(Path(c.PARENT_MANIFEST).read_text());parentok=parent.get('aggregate_valid') is True and parent['exact_results']['constructor_basis_class_indices']==c.PARENT_BASIS
